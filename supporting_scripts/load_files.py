...
def load_ply_data(ply_file):
    ...
    with open(ply_file, 'r') as f:
        ...
        faces_list_full = faces_list
        # print(faces_list_full)
        faces_list = np.array(faces_list)
        faces_list = faces_list[:,1:]
    return number_vertices[0], np.array(vertices_list), number_face[0], faces_list, faces_list_full
...
def open_ply(ply_file):
    try:
        ...
        faces_full = []
        print('Failed to read the file')
        return number_vertices, np.array(vertices_list), number_face, np.array(faces_list), faces_full, loaded_correctly
